...
LiquidCrystalDevice_t lq_init(uint8_t address, uint8_t columns, uint8_t rows,
                              uint8_t dotSize) {
  ...
  lq_transmitI2C(
      &device,
      LCD_NOBACKLIGHT);  // reset expanderand turn backlight off (Bit 8 =1)
  _delay_ms(1000);
...
void lq_clear(LiquidCrystalDevice_t *device) {
  ...
  lq_sendCommand(
      device, LCD_CLEARDISPLAY);  // clear display, set cursor position to zero
...
void lq_writeDeviceByte(LiquidCrystalDevice_t *device, uint8_t value,
                        uint8_t mode) {
