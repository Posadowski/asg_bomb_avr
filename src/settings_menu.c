...
void settings_menu(LiquidCrystalDevice_t* lcd)
{
    ...
    while (1)
    {
        ...
        if (screenChanged)
        {
           ...
           uint8_t i; 
            for (i = 0; i < LCD_LINE_LENGTH; i++) // TODO only clear to the end of the line
            {
               lq_print(lcd, " ");
            }
        ...
        if (old_key_pressed != key_pressed) {            
                if (key_pressed >= '0' && key_pressed <= '9')
                {
                    ...
                    {
                        if (settings_positionInCommandTable < (sizeof(settings_enteredCommandTable) / sizeof(settings_enteredCommandTable[0])))
                            lq_setCursor(lcd,1,settings_positionInCommandTable);
                            char print[] = {key_pressed,'\0'};
                       lq_print(lcd, print);
                        settings_enteredCommandTable[settings_positionInCommandTable] = (char)key_pressed;
                        settings_enteredCommandTable[++settings_positionInCommandTable] = '\0';
                    }
                }
                ...
                else if (key_pressed == (char)SETTINGS_CLEAR_KEY)
                {
                    ...
                    } else if (newValueToSet == TRUE)
                    {
                        if(settings_positionInCommandTable <=0){
                            ...
                            screenChanged = TRUE; // need to reload screen
                            newValueToSet = FALSE;
                        } else if (settings_positionInCommandTable < (sizeof(settings_enteredCommandTable) / sizeof(settings_enteredCommandTable[0])))
